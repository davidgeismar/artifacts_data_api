class LotsController < ApplicationController
  def show
    ...
    render json: {
      lot: lot,
      sales: format_lot_data(all_lot_sales),
      total: total,
      sales_count: all_lot_sales.count,
      performance: compute_object_performance(all_lot_sales)
    }
  end
  ...
  def compute_object_performance(all_lot_sales)
    all_lot_sales.reduce(0) {|sum, lot| sum + (lot.primary_price.to_f / lot.low_estimate.to_f) + (lot.primary_price.to_f / lot.high_estimate.to_f)} / all_lot_sales.count
  end
